...
- **Module.cryptoGetDescriptor(accountId) | Array\<CryptoDescriptor\>**  
  Returns the capabilities this crypto implementation supports, including allowed derivation paths

- **Module.cryptoOperate(accountId, payload, options) | JSON**  
  Used for signing and other crypto operations

- **Module.cryptoGetKey(accountId, path) | bytes | Error**  
  Derives a key at the given path — returns an error if the path is not allowed by the CryptoDescriptor
...
- **CRYPTO.getDescriptor(accountId) | Array\<CryptoDescriptor\>**  
  ...
  The website uses this to know what derivation paths and operations are available
...
- **CRYPTO.getKey(accountId, path) | bytes | Error**  
  Returns a derived key at the given path, or an error if path is not within the allowed range defined in the CryptoDescriptor
...
- **Module.cryptoGetKey(accountId, path) | bytes | Error**  
  Derives keys at BIP44/49/84/86 paths — returns error if path is outside allowed range
...
Child modules under address-book implement specific address list strategies:
...
The website decides which address list module to use, or it can use all available ones based on the CapabilityDescriptor.
...
- **AB.getCapabilities(accountId, cryptoGroupId) | Array\<CryptoDescriptor\>**  
  Returns the capabilities of the account-crypto combination  
  The website uses this to know which derivation paths and address types it can request

- **AB.getAddress(accountId, cryptoGroupId, derivationPath) | AddressInfo | Error**  
  Returns the address at the given derivation path  
  Returns an error if the path is not within the allowed range in the CryptoDescriptor  
  ***derivationPath***: e.g. `m/84'/0'/0'/0/0` | *String*
...
- **AB.exportDescriptors(accountId, cryptoGroupId) | Array\<CryptoDescriptor\>**  
  Returns the full capability descriptor for the account-crypto combination  
  Allows the website to understand what the wallet can do and which paths are accessible
