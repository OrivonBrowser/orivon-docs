...
- **Module.getAccountDescriptor(accountId) | Array\<CapabilityDescriptor\>**  
  Must include capability: `SECP256K1_SIGN`
...
Manages derived addresses across all supported crypto groups.
Uses *CapabilityDescriptor* to determine which crypto standards an account supports.
...
- Any compatible crypto group (e.g. *group.crypto.btc0-1*, *group.crypto.eth0-1*)

#### Provides access to ->

- **AB.getAddresses(accountId, cryptoGroupId, chain, count) | Array\<AddressInfo\>**  
  Returns derived addresses for the given account and crypto standard  
...
- **AB.getSupportedCryptos(accountId) | Array\<string\>**  
  Returns all crypto group IDs the account is compatible with
