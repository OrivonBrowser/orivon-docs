...
An [**Application**](/docs/orivon#applications--also-called-special-extensions-) can implement support for one or more first level domains

DNS Resolutions Apps are required to return the entire DNS Record list, so that the [**Data Gathering**](/docs/implementations/data-gathering) app can use it for its needs
